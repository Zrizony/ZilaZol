...
# -*- coding: utf-8 -*-
"""
crawler_enhanced_local.py  –  Local test version of enhanced crawler

This version runs locally without requiring Google Cloud services
"""
...
LOCAL_OUTPUT_DIR = "enhanced_output"
LOCAL_DOWNLOADS_DIR = "enhanced_downloads"
...
# ───────────── LOCAL DATA STORAGE ────────────────────────────────────
def save_product_data(shop: str, product_data: dict, source_file: str):
    """Save product data to local JSON file with versioning"""
    try:
        # Create shop directory
        shop_dir = os.path.join(LOCAL_OUTPUT_DIR, slug(shop))
        os.makedirs(shop_dir, exist_ok=True)
        
        # Create products file
        products_file = os.path.join(shop_dir, "products.json")
        ...
        if doc_id in products:
            # Update existing product
            current_data = products[doc_id]
            ...
            last_update_str = current_data.get('last_updated')
            ...
            should_add_history = (
                ...
                (datetime.now() - last_update).total_seconds() > 3600  # More than 1 hour ago
            )
            ...
            if should_add_history:
                ...
                cutoff_date = datetime.now() - timedelta(days=PRICE_HISTORY_DAYS)
        ...
        log.error(f"Failed to save product {product_data['barcode']} in {shop}: {e}")
        return False
    ...
    try:
        if shop:
            ...
            if os.path.exists(products_file):
                ...
                doc_id = f"{shop}_{barcode}"
                if doc_id in products:
                    data = products[doc_id]
                    cutoff_date = datetime.now() - timedelta(days=days)
                    history = [p for p in data.get('price_history', []) 
                             if datetime.fromisoformat(p['date']) > cutoff_date]
                    return history
            ...
            for shop_dir in os.listdir(LOCAL_OUTPUT_DIR):
                products_file = os.path.join(LOCAL_OUTPUT_DIR, shop_dir, "products.json")
                if os.path.exists(products_file):
                    with open(products_file, 'r', encoding='utf-8') as f:
                        products = json.load(f)
                    
                    # Find products with matching barcode
                    for doc_id, data in products.items():
...
def save_file_locally(shop: str, filename: str, data: bytes):
    """Save file to local storage"""
    try:
        shop_dir = os.path.join(LOCAL_DOWNLOADS_DIR, slug(shop))
        os.makedirs(shop_dir, exist_ok=True)
        ...
        log.info(f"Saved {filename} to {shop_dir}")
        return True
    except Exception as e:
...
def playwright_with_login(hub: str, creds: dict | None, shop: str = "retailer"):
    ...
    screenshot_filename = f"login_{slug(shop_str)}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    
    # Take screenshot
    screenshot_data = page.screenshot(full_page=True)
    
    # Save screenshot locally
    save_file_locally(shop_str, screenshot_filename, screenshot_data)
    log.info(f"Saved login screenshot: {screenshot_filename}")
...
def process_and_store_data(shop: str, xml_data: bytes, source_file: str):
    ...
    for row in rows:
        try:
            ...
            if save_product_data(shop, row, source_file):
                processed_count += 1
            else:
                error_count += 1
...
def test_local_setup():
    """Test local setup"""
    log.info("Testing local setup...")
    
    # Test directory creation
    try:
        os.makedirs(LOCAL_OUTPUT_DIR, exist_ok=True)
        os.makedirs(LOCAL_DOWNLOADS_DIR, exist_ok=True)
        log.info("✅ Local directories created")
    except Exception as e:
    ...
    # Test file writing
    try:
        test_file = os.path.join(LOCAL_OUTPUT_DIR, "test.json")
        with open(test_file, 'w') as f:
            json.dump({"test": "data"}, f)
        os.remove(test_file)
        log.info("✅ File I/O test passed")
    except Exception as e:
        log.error(f"❌ File I/O test failed: {e}")
        return False
    
    log.info("✅ All local setup tests passed")
    return True

def main():
    # Test local setup first
    if not test_local_setup():
        log.error("Local setup failed, exiting")
        return
    ...
    for shop, hub in retailer_links().items():
        ...
        for url in files:
            try:
                ...
                if save_file_locally(shop_str, fname, resp.content):
                    counts["zips"] += 1
